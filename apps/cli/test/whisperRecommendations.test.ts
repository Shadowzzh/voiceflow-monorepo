...
describe('Whisper 推荐配置', () => {
  it('应该基于实际硬件生成合适的推荐配置', async () => {
    ...
    console.log(`GPU: ${hardware.gpu ? `${hardware.gpu.vendor} ${hardware.gpu.model}` : '无'}`)
    console.log(`磁盘: ${hardware.disk.total}GB 总容量，${hardware.disk.available}GB 可用`)
    ...
    expect(['base', 'small', 'medium', 'large']).toContain(recommendations.model)
    expect(['cpu', 'cuda', 'opencl', 'metal']).toContain(recommendations.backend)
