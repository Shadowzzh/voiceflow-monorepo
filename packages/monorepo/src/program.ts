...
program
  .command('init')
  .description('初始化 package.json 和 README.md')
  .action(async () => {
    await init(cwd)
...
program
  .command('sync')
  .description('向 npmmirror 同步所有子包')
  .action(async () => {
...
program
  .command('mirror')
  .description('设置 VscodeBinaryMirror')
  .action(async () => {
...
program
  ...
  .action(
    async (
      name: string,
      options: { tsup?: boolean; unbuild?: boolean; vueUi?: boolean }
    ) => {
      if (!name) {
        name = await input({
          message: '请输入包名',
